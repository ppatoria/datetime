...
#pragma pack(pop)
        ...
        std::uint16_t builtinCapacity_() const
        {
            ...
            const std::uint16_t capacity(
                capacityFlags == 0 /* 0000 */ ? CADRE_STRING_MIN_BUILTIN_CAPACITY : (capacityFlags == 1 /* 0001 */ ? 2 * CADRE_STRING_MIN_BUILTIN_CAPACITY : (capacityFlags == 2 /* 0010 */ ? 256 : (capacityFlags == 3 /* 0011 */ ? CADRE_STRING_MAX_BUILTIN_CAPACITY : 0))));
        ...
        constexpr void construct_(const BuiltinCapacity<CAPACITY>&)
        {
            ...
            static_assert(
                CAPACITY == CADRE_STRING_MIN_BUILTIN_CAPACITY || CAPACITY == 2 * CADRE_STRING_MIN_BUILTIN_CAPACITY || CAPACITY == 256 || CAPACITY == CADRE_STRING_MAX_BUILTIN_CAPACITY,
                "Invalid CAPACITY");

            _flags = (CAPACITY == CADRE_STRING_MIN_BUILTIN_CAPACITY ? 0 /* 0000 */ : (CAPACITY == 2 * CADRE_STRING_MIN_BUILTIN_CAPACITY ? 1 /* 0001 */ : (CAPACITY == 256 ? 2 /* 0010 */ : (CAPACITY == CADRE_STRING_MAX_BUILTIN_CAPACITY ? 3 /* 0011 */ : 0))));
    ...
    public:
        ...
        {
            if (size() != length || *c_str() != *sz || 0 != std::strncmp(c_str(), sz, length)) {
                return false;
            }
        ...
        {
            return pattern.size() <= size() && 0 == std::strncmp(c_str(), pattern.c_str(), pattern.size());
        }

        bool ends_with(const String& pattern) const
        {
            return pattern.size() <= size() && 0 == std::strncmp(rbegin() - pattern.size() + 1, pattern.c_str(), pattern.size());
        }
    ...
    public:
        ...
        explicit RegularString(char from)
            : String(BuiltinCapacity<BUFFER_SIZE>())
        {
            assign(&from, 1);
        }
    ...
    public:
        ...
        explicit MediumString(char from)
            : RegularString(BuiltinCapacity<BUFFER_SIZE>())
        {
            assign(&from, 1);
        }
